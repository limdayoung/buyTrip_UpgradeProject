						...
						<div class="wrapper row">
							<div class="preview col-md-6">
								...
								<div class="preview-pic tab-content">
											...
											<div class="section-content">
												<div class="product-details">
													<ul class="product-images">
														<li class="preview"><img src="${imgList[0]}" alt="">
														</li>
									...
									<p class="product-description">상세 설명 :
										${productDTO.productDesc}</p>
									<div class="well" style="font-size: 15px">요구사항 :
										${productDTO.requirement}</div>
								</div>
							...
							<div class="details col-md-6">
								...
								<div style="align-self: center;">
									<p class="product-description">마감 기한 :
										${productDTO.deadlineDate}</p>
								</div>
								...
								<h3 class="price product-title">
									제안 금액 <span><fmt:formatNumber
											value="${productDTO.productPrice}" />원</span>
								</h3>
								<h4 class="price product-title">
									환율<span>${productDTO.nationCurrency}<fmt:formatNumber
											value="${productDTO.ceil}" /></span>
								</h4>
								
								
								<p>
								<h4>
									갯수 <strong>${productDTO.productQty} </strong>
								</h4>
								</p>
								<br>

								<div class="well" style="font-size: 15px">
									<strong>여행나라</strong> ${productDTO.arrivalNation}
									<hr>
									<strong>출발지</strong> ${productDTO.departNation}
								</div>
								<c:if test="${productDTO.proposerId != memberId}">
						...
						<c:forEach items="${requestScope.offerList}" var="userDTO">
							...
							<div class="card">
								<div class="well">
									<div class="row">
										<div class="col-lg-2">
											<img src=" ${userDTO.memberImg }" class="avatar img-circle"
												alt="avatar" style="width: 100px; height: auto;">
										</div>
										<div class="col-lg-10">
											<div class="row">
												<div class="col-xs-4">
										...
										<c:if test="${userDTO.memberId != memberId}">
									
										<form action="${pageContext.request.contextPath }/chatForm" id="chatForm" method="post" name="chatForm">
										<input type="hidden" name="userId" value="${userDTO.memberId}">
										<input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}" />
										<a href="#" class="btn btn-info" role="button"style="width: 150px;" onclick="buttonclick()">메세지 하기</a>
										</form> 
		...
		var Chef = {
			...
			menuToggle : function() {
				$(document).on(
						...
						function() {
							...
							// Toggle menu links
							$(this).siblings('.links').stop(true, true)
									.slideToggle(200);
			...
			misc : function() {
				...
				for (var i = 1; i <= 3; i++) {
					...
					$('.product').parent().eq(0).clone().appendTo(
							'.product-list');
				}
